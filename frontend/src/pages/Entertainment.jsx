...
const Entertainment = () => {
    ...
    const fetchNews = async () => {
      try {
        const response = await fetch(
          `https://newsapi.org/v2/top-headlines?category=entertainment&language=en&pageSize=30&apiKey=${import.meta.env.VITE_API_KEY}`
        );
  ...
  // Categories for entertainment news
  const categories = [
  ...
  // Filter articles based on category (in a real app, you would fetch filtered data)
  // Here we're simulating it by checking if article titles contain category keywords
  const filteredNews = currentCategory === 'all' 
    ...
    : news.filter(article => {
        ...
        switch(currentCategory) {
          case 'movies':
            return searchText.includes('movie') || searchText.includes('film') || searchText.includes('cinema');
          case 'music':
            return searchText.includes('music') || searchText.includes('song') || searchText.includes('album') || searchText.includes('artist');
          case 'tv':
            return searchText.includes('tv') || searchText.includes('television') || searchText.includes('series') || searchText.includes('show');
          case 'celebrity':
            return searchText.includes('celebrity') || searchText.includes('star') || searchText.includes('actor') || searchText.includes('actress');
          default:
            return true;
        }
      });
    ...
    if (!filteredNews || filteredNews.length === 0) {
      return (
        <div className="text-center py-12">
          <h2 className="text-2xl font-semibold text-blue-700">No entertainment news available at the moment.</h2>
      ...
      >
        ...
        {featuredArticle && (
          ...
          >
            ...
            <img 
              src={featuredArticle.urlToImage || entertainment} 
          ...
          {regularArticles.map((article, index) => (
            ...
            >
              <div className="relative h-56 overflow-hidden">
                <img 
                  src={article.urlToImage || entertainment} 
                  alt={article.title}
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
            ...
            >
              <div className="relative">
                <img 
                  src={activeArticle.urlToImage || entertainment} 
